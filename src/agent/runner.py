...
from .rag_memory import format_retrieved_examples_for_prompt

# Import VLM API
try:
    from .api import call_vlm
    VLM_AVAILABLE = True
except ImportError:
    VLM_AVAILABLE = False
...
PHASE1_RESPONSE_SCHEMA: Dict[str, Any] = {
    "type": "object",
    "properties": {
        "action": {"type": "string", "enum": ["KEEP", "DISCARD", "SPLIT"]},
...
PHASE2_RESPONSE_SCHEMA: Dict[str, Any] = {
    "type": "object",
    "properties": {
        "action": {"type": "string", "enum": ["MERGE", "NOT_MERGE", "DISCARD"]},
        "rationale": {"type": "string"},
    },
    "required": ["action", "rationale"],
    "additionalProperties": False,
...
NEURONAL_RESPONSE_SCHEMA: Dict[str, Any] = {
    "type": "object",
    "properties": {
...
def _load_extra_body_from_env() -> Optional[Dict[str, Any]]:
    """Load optional OpenAI-compatible extra_body JSON for provider-specific knobs."""
    raw = os.getenv("VLM_EXTRA_BODY_JSON", "").strip()
    if not raw:
        return None
    try:
        ...
        pass
    print("[VLM API] Ignoring invalid VLM_EXTRA_BODY_JSON; expected JSON object")
    return None
...
def call_vlm_api(
    ...
    Args:
        ...
        provider: API provider ("gpt4o", "openrouter", "vllm", or "claude")
    ...
    if use_mock or not VLM_AVAILABLE:
        ...
        if "simulating expert human feedback" in prompt.lower():
            return "Looks reasonable overall. Focus feedback on the specific mismatch between your action and the evidence."
    ...
    extra_body = _load_extra_body_from_env()

    # Real API call with retry
    for attempt in range(max_retries):
        try:
            # Detect reasoning model
            is_reasoning = any(rm in model for rm in ["o1", "o3", "gpt-5"])
...
def vlm_neuronal_validity_check(
    ...
    """
    from .context import NEURONAL_CRITERIA
...
"""
    ...
    for parse_attempt in range(max_parse_attempts):
        raw_response = call_vlm_api(
            ...
            response_schema=NEURONAL_RESPONSE_SCHEMA,
        )
...
def vlm_phase1_cluster_decision(
    ...
    Args:
        ...
        retrieved_examples: Optional retrieved few-shot examples from continual memory.
    ...
    Returns:
        Dict with:
            ...
            - image_paths: Saved image paths (if output_dir is provided)
    """
    ...
    if retrieved_examples:
        prompt = (
            prompt.rstrip()
            + "\n\n"
            + format_retrieved_examples_for_prompt(retrieved_examples)
            + "\n"
        )
    
    # Save inputs if output_dir provided
    save_meta = _save_vlm_inputs(
        ...
        extra_meta={
            "rag_enabled": int(bool(retrieved_examples)),
            "rag_hits": len(retrieved_examples or []),
        },
    )
    ...
    for parse_attempt in range(max_parse_attempts):
        ...
        try:
            ...
            return {
                ...
                "raw_response": raw_response,
                "prompt_text": prompt,
                "image_paths": [] if save_meta is None else save_meta.get("image_files", []),
            }
        except Exception as e:
            print(f"[VLM Parse Error] Attempt {parse_attempt + 1}/{max_parse_attempts}: {e}")
            print(f"[VLM Raw Response] {raw_response[:500]}...")
