...
%   Last Modified By: sbm on Thu Oct  6 20:29:54 2005
...
%% shreesh: 4.9.2012. the next line added to deal with cases in which there
%% are no spikes (for instance, in th eImc channel, after Imc
%% inactivation).
if isempty(tree), tree=[1 1 1]; end
...
for step = 1:aggsteps
    ...
    %%-- Added by Shreesh 9.10.09
    % this means one of the two children is the outlier cluster,
    % which we dont show in the traditional agg tree format. so we omit it.
    if lchild~=0 && rchild~=0
    %end Added by Shreesh 9.10.09 --%%
...
for root = 1:length(treeroots)
	...
	text(x1 - 0.61, y2*1.15 + stagger,    ['Clust #' num2str(oldname)], 'Color', [0 0 0], 'FontSize', 15);
    text(x1 - 0.59, y2*1.03 + stagger,    ['N=' num2str(clustsize)], 'Color', [1 0 0], 'FontSize', 15);
    text(x1 + 0.22, y2*0.90 + stagger/3,  ['isi=' num2str(scores(1),2)], 'Color', [0 0 1], 'FontSize', 15);
end
