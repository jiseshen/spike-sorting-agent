...
if (isfield(spikes, 'overcluster') && all(ismember(assignments, spikes.overcluster.assigns)))
    cmap = spikes.overcluster.colors(clusters(clusters>0,:),:);
else
...
for clu = 1:numclusts
	...
	if (clusters(clu) == 0),                 set(hndl, 'Color', [0.2 0.2 0.2], 'Marker', 'x');
	elseif (ismember(clusters(clu), show)),  set(hndl, 'Color', cmap(clu-1,:));
...
if (length(h_pts) == 1), set(h_pts, 'MarkerSize', 5, 'Marker', '.');  end;
