...
%   Last Modified By: sbm on Thu Oct  6 20:30:34 2005
...
elseif (~all(ismember([to,from], unique([0; spikes.hierarchy.assigns]))))
    missing_label = [to,from]; missing_label = missing_label(~ismember([to,from], unique([0; spikes.hierarchy.assigns])));
    error('SS:cluster_numbers_not_found', ['One or both of the cluster labels supplied does not exist (',num2str(missing_label),' of command m ',num2str(to),' ',num2str(from),')']);
...
if (isfield(spikes, 'spiketimes'))   % might as well compute the isi score if we can ...
    ...
    [allow, score] = isiQuality(t1, t2, tmin, 0.01, tref, spikes.Fs);
    score = score(3);
