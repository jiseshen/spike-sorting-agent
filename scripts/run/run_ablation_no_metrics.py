"""
ABLATION TEST: Run VLM curation WITHOUT quality metrics in prompts.

Tests whether VLM can make accurate curation decisions based on visual
information (images) plus cluster size info, but WITHOUT quality metrics like:
- ISI violation rates (numerical values)
- Waveform correlation scores (numerical values)
- Amplitude CV values (numerical values)

KEEPS in prompts:
- Spike counts (cluster sizes)
- Overcluster counts (structural complexity)
- Total spikes after merge
- Qualitative descriptions of what to look for

Runs on all channels with gpt-5.1 model in parallel.
Output: output/ablation_no_metrics/
"""
...
from src.ablation.pipeline import PureVLMCurationPipeline
...
PROVIDER = "gpt4o"
MODEL = "gpt-5.1"  # Use gpt-5.1 for ablation test
TEMPERATURE = 0.0
...
# Output directory
OUTPUT_BASE = Path("output/ablation_no_metrics")
OUTPUT_BASE.mkdir(parents=True, exist_ok=True)

print("="*80)
print("ABLATION TEST: VLM WITHOUT QUALITY METRICS")
print("="*80)
print("This test removes quality metrics from VLM prompts.")
print("VLM makes decisions based on images + cluster size info.")
print()
print("Removed from prompts:")
print("  - ISI violation rates (numerical values)")
print("  - Waveform correlation scores (numerical values)")
print("  - Amplitude CV values (numerical values)")
print("  - Quality thresholds (e.g., '> 0.6%')")
print()
print("Kept in prompts:")
print("  - Spike counts (cluster sizes)")
print("  - Overcluster counts (structural complexity)")
print("  - Total spikes after merge")
print("  - Images (waveform overlays, ISI histograms, aggregation trees)")
print("  - Qualitative decision criteria")
print("  - Neuronal shape requirements")
print("="*80)
...
def process_channel(channel):
    """
    Process a single channel with ablation pipeline.
    ...
    # [4] Initialize ABLATION pipeline
    print(f"\n[4] Initializing ABLATION pipeline (no metrics in prompts)...")
    ...
    print(f"  ✓ Ablation pipeline initialized")
    
    # [5] Run pipeline
    print(f"\n[5] Running ablation pipeline...")
    if USE_MOCK:
        print(f"  📝 Using MOCK VLM responses (no API calls)")
    else:
        print(f"  🌐 Using REAL VLM API: {PROVIDER}/{MODEL}")
        print(f"  ⚠ VLM receives NO numerical metrics (visual-only decisions)")
...
if __name__ == '__main__':
    print("="*80)
    print("ABLATION TEST: VLM WITHOUT QUANTITATIVE METRICS (PARALLEL)")
    print("="*80)
    print(f"Provider: {PROVIDER}")
    print(f"Model: {MODEL}")
    print(f"Channels: {', '.join(CHANNELS)}")
    ...
    print("-"*80)
    print("Run `uv run python scripts/aggregate/aggregate_results.py --base-dir output/ablation_no_metrics` to compute cross-channel summaries when desired.")
    if channel_stats:
    ...
    print("Next: aggregate with `uv run python scripts/aggregate/aggregate_results.py --base-dir output/ablation_no_metrics` if needed.")
    print("="*80)
